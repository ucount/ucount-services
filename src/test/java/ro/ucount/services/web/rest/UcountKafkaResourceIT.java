...
import ro.ucount.services.IntegrationTest;
import ro.ucount.services.config.EmbeddedKafka;
import ro.ucount.services.config.KafkaSseConsumer;
import ro.ucount.services.config.KafkaSseProducer;

@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
@EmbeddedKafka
class UcountKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ucount-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ucount-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/ucount-kafka/unregister"));
                return;
            }
        }
